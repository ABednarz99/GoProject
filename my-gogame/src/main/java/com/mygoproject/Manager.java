...
public class Manager {
    ...
    public boolean addStoneToChain(int x, int y, String color, Player player) {
    ...
    public void sendRemovedStones(int index) {
        PrintWriter out1 = currentPlayer.output;
        PrintWriter out2 = currentPlayer.opponent.output;
        
        for(int i = 0; i < chains.get(index).numberOfElements(); i++) {
            int a = chains.get(index).getX(i);
            int b = chains.get(index).getY(i);
            out1.println("REMOVE" + a + " " + b);
            out2.println("REMOVE" + a + " " + b);
        }
    }
